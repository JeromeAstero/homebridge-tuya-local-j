class BaseAccessory {
    constructor(...props) {
        ...
        this.device.once('change', () => {
            this.log(`Ready to handle device{name=${this.device.context.name} (${this.device.context.type}:${this.device.context.version}), id=${this.device.context.id}, ip=${this.device.context.ip}}`);
