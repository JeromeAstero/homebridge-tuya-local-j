...
class RGBWLightAccessory extends BaseAccessory {
    ...
    getBrightness(callback) {
        if (this.device.state[this.dpMode] === this.cmdWhite) {
            this.log.debug(`[method]getBrightness(), device state=${this.device.state[this.dpMode]}, return=${this.convertBrightnessFromTuyaToHomeKit(this.device.state[this.dpBrightness])}`);
            ...
            this.log.debug(`[method]getBrightness(), device state=${this.device.state[this.dpMode]}, return=${this.convertColorFromTuyaToHomeKit(this.device.state[this.dpColor]).b}`);
    ...
    setBrightness(value, callback) {
        if (this.device.state[this.dpMode] === this.cmdWhite) {
            this.log.debug(`[method]setBrightness(), value=${value}, dpBrightness=${this.dpBrightness}, return=${this.convertBrightnessFromHomeKitToTuya(value)}`);
            return this.setState(this.dpBrightness, this.convertBrightnessFromHomeKitToTuya(value), callback);
        } else if (this.device.state[this.dpMode] === this.cmdColor) {
            this.log.debug(`[method]setBrightness(), value=${value}, dpColor=${this.dpColor}, return=${this.convertColorFromHomeKitToTuya({b: value})}`);
    ...
    setColorTemperature(value, callback) {
        this.log.debug(`[method]setColorTemperature(), value=${value}`);
        ...
        this.log.debug(`[method]setColorTemperature(), dpBrightness=${currentBrightness}`);
        this.log.debug(`[method]setColorTemperature(), characteristicBrightness=${newColor.b}`);
