...
class RGBWLightAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.device.on('change', (changes, state) => {
            setTimeout(() => {
                // Ваша логика синхронизации характеристик
            }, 100); // Задержка в 100 мс (или больше, в зависимости от устройства)
            if (changes.hasOwnProperty(this.dpPower) && characteristicOn.value !== changes[this.dpPower]) {
                ...
                if (changes[this.dpPower]) { // Если устройство включено
                    ...
                    if (state[this.dpMode] === this.cmdWhite) {
                        ...
                        const colorTemperature = this.convertColorTemperatureFromTuyaToHomeKit(state[this.dpColorTemperature] || 400);
            ...
            switch (state[this.dpMode]) {
                ...
                case this.cmdWhite: {
                    if (changes.hasOwnProperty(this.dpBrightness)) { // Проверка изменения яркости
                    ...
                    if (changes.hasOwnProperty(this.dpColorTemperature)) {
                        const newColorTemperature = this.convertColorTemperatureFromTuyaToHomeKit(400);
                        // const newColor = this.convertHomeKitColorTemperatureToHomeKitColor(newColorTemperature);
                        characteristicHue.updateValue(0); // Hue на 0 для белого света
                        characteristicSaturation.updateValue(0); // Saturation на 0 для белого света
                        characteristicColorTemperature.updateValue(newColorTemperature);
                    }
                    break;
                }
    ...
    /*    setBrightness(value, callback) {
            if (!this.device.state[this.dpPower]) { // Включаем устройство
                this.log.debug(`device{name=${this.device.context.name}, id=${this.device.context.id}, ip=${this.device.context.ip}}; [method]setBrightness(), device.power=${this.device.state[this.dpPower]}`);
                this.setState(this.dpPower, true, callback);
            }
            if (this.device.state[this.dpMode] === this.cmdWhite) {
                this.log.debug(`device{name=${this.device.context.name}, id=${this.device.context.id}, ip=${this.device.context.ip}}; [method]setBrightness(), device.mode=${this.device.state[this.dpMode]}, cmd=${this.cmdWhite}, value=${value}, dpBrightness=${this.dpBrightness}, return=${this.convertBrightnessFromHomeKitToTuya(value)}`);
                return this.setState(this.dpBrightness, this.convertBrightnessFromHomeKitToTuya(value), callback);
    ...
    setColorTemperature(value, callback) {
        ...
        // this.log.debug(`device{name=${this.device.context.name}, id=${this.device.context.id}, ip=${this.device.context.ip}}; [method]setColorTemperature(), dpBrightness=${currentBrightness}, {h:${newColor.h},s:${newColor.s},b:${newColor.b}}, currentBrightness=${currentBrightness}`);
        ...
        this.setMultiState({
            ...
            [this.dpBrightness]: currentBrightness + 1 // Фикс включения белого режима
        }, callback);
    }
