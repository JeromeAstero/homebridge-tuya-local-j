...
class RGBWLightAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        const characteristicColorTemperature = service.getCharacteristic(Characteristic.ColorTemperature)
            ...
            .updateValue(dps[this.dpMode] === this.cmdWhite ? 0 : 0)
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(this.dpMode) && characteristicColorTemperature.value !== changes[this.dpMode]) {
            ...
            switch (state[this.dpMode]) {
                case this.cmdWhite:
                    ...
                    if (changes.hasOwnProperty(this.dpColorTemperature) && this.convertColorTemperatureFromHomeKitToTuya(characteristicColorTemperature.value) !== changes[this.dpColorTemperature]) {
                        ...
                        const newColorTemperature = this.convertColorTemperatureFromTuyaToHomeKit(0);
    ...
    getBrightness(callback) {
        if (this.device.state[this.dpMode] === this.cmdWhite) {
            this.log.info('return', this.convertBrightnessFromTuyaToHomeKit(this.device.state[this.dpBrightness]));
            return callback(null, this.convertBrightnessFromTuyaToHomeKit(this.device.state[this.dpBrightness]))
        }
        this.log.info('return', this.convertColorFromTuyaToHomeKit(this.device.state[this.dpColor]).b);
    ...
    setBrightness(value, callback) {
        if (this.device.state[this.dpMode] === this.cmdWhite) {
            this.setMultiState({
                ...
                [this.dpBrightness]: this.convertBrightnessFromHomeKitToTuya(value)
            }, callback);
    ...
    setColorTemperature(value, callback) {
        ...
        const newColorTemperature = 400; // Белый свет всегда фиксированный
        this.setMultiState({
            [this.dpMode]: this.cmdWhite,
            [this.dpBrightness]: this.convertBrightnessFromHomeKitToTuya(this.characteristicBrightness.value || 100)
        }, callback);

        this.characteristicColorTemperature.updateValue(newColorTemperature);
        this.characteristicHue.updateValue(0);
        this.characteristicSaturation.updateValue(0);
    }
