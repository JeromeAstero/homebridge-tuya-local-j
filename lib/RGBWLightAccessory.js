...
class RGBWLightAccessory extends BaseAccessory {
    ...
    getBrightness(callback) {
        if (this.device.state[this.dpMode] === this.cmdWhite) {
            this.log.debug(`device{name=${this.device.context.name}, id=${this.device.context.id}, ip=${this.device.context.ip}}; [method]getBrightness(), device state=${this.device.state[this.dpMode]}, return=${this.convertBrightnessFromTuyaToHomeKit(this.device.state[this.dpBrightness])}`);
            ...
            this.log.debug(`device{name=${this.device.context.name}, id=${this.device.context.id}, ip=${this.device.context.ip}}; [method]getBrightness(), device state=${this.device.state[this.dpMode]}, return=${this.convertColorFromTuyaToHomeKit(this.device.state[this.dpColor]).b}`);
    ...
    setColorTemperature(value, callback) {
        this.log.debug(`device{name=${this.device.context.name}, id=${this.device.context.id}, ip=${this.device.context.ip}}; [method]setColorTemperature(), value=${value}`);
        ...
        this.log.debug(`device{name=${this.device.context.name}, id=${this.device.context.id}, ip=${this.device.context.ip}}; [method]setColorTemperature(), dpBrightness=${currentBrightness}, characteristicBrightness=${newColor.b}, currentBrightness=${currentBrightness}`);

        this.setMultiState({
            [this.dpMode]: this.cmdWhite,
            [this.dpBrightness]: this.convertBrightnessFromHomeKitToTuya(currentBrightness || 33) // Установка текущей или максимальной яркости
        }, callback);
    }
