...
class RGBWLightAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        const characteristicColorTemperature = service.getCharacteristic(Characteristic.ColorTemperature)
            ...
            .updateValue(dps[this.dpMode] === this.cmdWhite ? 500 : 0)
            ...
            switch (state[this.dpMode]) {
                case this.cmdWhite:
                    if (changes.hasOwnProperty(this.dpBrightness) && this.convertBrightnessFromHomeKitToTuya(characteristicBrightness.value) !== changes[this.dpBrightness]) {
                        this.log.info('switch (state[this.dpMode]) {');
                        characteristicBrightness.updateValue(this.convertBrightnessFromTuyaToHomeKit(changes[this.dpBrightness]));
                        characteristicColorTemperature.updateValue(500);
                    }
                    if (changes[this.dpMode]) {
                ...
                default:
                    if (changes.hasOwnProperty(this.dpColor)) {
                        ...
                        if (characteristicColorTemperature.value !== 0) characteristicColorTemperature.updateValue(500);
    ...
    setColorTemperature(value, callback) {
        this.log.info('[start]: setColorTemperature(value, callback)');
        this.log.info(`currentDPMode: ${this.device.state[this.dpMode]}`);
        this.log.info(`currentBright: ${this.device.state[this.dpBrightness]}`);
        this.setMultiState({[this.dpMode]: this.cmdWhite, [this.dpBrightness]: this.device.state[this.dpBrightness]+1, [this.dpBrightness]: this.device.state[this.dpBrightness]-1}, callback);
        this.log.info(`newDPMode: ${this.device.state[this.dpMode]}`);
        this.log.info(`newBright: ${this.device.state[this.dpBrightness]}`);
        // this.setState(this.dpBrightness, this.device.state[this.dpBrightness], callback);
        this.log.info('[end]: setColorTemperature(value, callback)');
    }
    ...
    _setHueSaturation(prop, callback) {
        ...
        const callEachBack = err => {
            async.eachSeries(callbacks, (callback, next) => {
                ...
                this.characteristicColorTemperature.updateValue(500);
            });
        };
