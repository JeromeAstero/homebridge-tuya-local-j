...
class RGBTWLightAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        const characteristicColorTemperature = service.getCharacteristic(Characteristic.ColorTemperature)
            ...
            .updateValue(dps[this.dpMode] === this.cmdWhite ? this.convertColorTemperatureFromTuyaToHomeKit(_GColorTemp) : 0)
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (characteristicOn.value !== changes[this.dpPower]) {
                characteristicOn.updateValue(changes[this.dpPower]);
            }

            // Вызов обработчика режима устройства
            // Изменения "белого" режима
            if (state[this.dpMode] === this.cmdWhite) {
                // Изменения яркости (dpBrightness)
                if (this.convertBrightnessFromHomeKitToTuya(characteristicBrightness.value) !== changes[this.dpBrightness]) {
                    characteristicBrightness.updateValue(this.convertBrightnessFromTuyaToHomeKit(changes[this.dpBrightness]));
                }
                // Изменение цветовой температруы (фейк)
                if (this.convertColorTemperatureFromHomeKitToTuya(characteristicColorTemperature.value) !== _GColorTemp) {

                    const newColorTemperature = this.convertColorTemperatureFromTuyaToHomeKit(_GColorTemp);
            ...
            // Изменения "цветного" режима
                ...
                if (oldColor.h !== newColor.h) characteristicHue.updateValue(newColor.h);           //hue
                if (oldColor.s !== newColor.s) characteristicSaturation.updateValue(newColor.s);    //saturation
                if (oldColor.b !== newColor.b) characteristicBrightness.updateValue(newColor.b);    //brightness
    ...
    getColorTemperature(callback) {
        ...
        callback(null, this.convertColorTemperatureFromTuyaToHomeKit(_GColorTemp));
    }
    ...
    setColorTemperature(value, callback) {
        ...
        const newBrightness = this.device.state[this.dpBrightness] + 1;
        this.characteristicHue.updateValue(newColor.h);
        this.characteristicSaturation.updateValue(newColor.s);
        _GColorTemp = this.convertColorTemperatureFromHomeKitToTuya(value);
        ...
        this.setMultiState({[this.dpMode]: this.cmdWhite, [this.dpBrightness]: newBrightness}, callback);
    }
