...
class RGBWLightAccessory extends BaseAccessory {
    ...
    getBrightness(callback) {
        if (this.device.state[this.dpMode] === this.cmdWhite) {
            this.log.info(`device:${this.device}`);
            this.log.info(`device context:${this.device.context}`);
            console.dir(this.device, { depth: null, colors: true });
            console.dir(this.device.context, { depth: null, colors: true });
