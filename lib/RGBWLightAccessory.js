...
class RGBWLightAccessory extends BaseAccessory {
    ...
    setColorTemperature(value, callback) {
        ...
        const tasks = [
            (cb) => this.setState(this.dpMode, this.cmdWhite, cb),
            (cb) => this.setState(this.dpBrightness, this.convertBrightnessFromHomeKitToTuya(value), cb)
        ];

        async.series(tasks, (err) => {
            if (err) {
                this.log.error('Error setting color temperature:', err);
                callback(err);
            } else {
                callback();
            }
